...
class Embedding(nn.Module):
    def __init__(self, in_channels, out_channels, in_size):
        ...
        self.embedding_layer = nn.Sequential(
            nn.Conv2d(in_channels, out_channels, kernel_size=in_size),
            nn.ReLU(inplace=True)
        )
...
class UpEmbedding(nn.Module):
    def __init__(self, in_channels, out_channels, out_size):
        super().__init__()
        self.up_embedding_layer = nn.Sequential(
            nn.ConvTranspose2d(in_channels, out_channels, kernel_size=out_size),
            nn.ReLU()
        )

    def forward(self, x):
        return self.up_embedding_layer(x)
